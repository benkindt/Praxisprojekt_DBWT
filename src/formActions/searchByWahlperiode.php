...
// 	$query = "SELECT * FROM person, gremiumsmitglied, gremium, wahlperiode WHERE person.pid IN (SELECT DISTINCT person.pid FROM person, gremiumsmitglied, gremium, wahlperiode WHERE wahlperiode.wid = '" . $wahlperiode . "') AND gremiumsmitglied.wid = wahlperiode.wid AND person.pid = gremiumsmitglied.pid ORDER BY person.pid;";
	$query = "SELECT DISTINCT ON (gremiumsmitglied.gmid) person.pid, person.vorname, person.nachname, gremiumsmitglied.gmid, gremium.name, gremiumsmitglied.nachruecker, gremiumsmitglied.von as gvon, gremiumsmitglied.bis as gbis, gremiumsmitglied.grund, wahlperiode.von as wvon, wahlperiode.bis as wbis, fachschaft.name as fname FROM person, gremiumsmitglied, gremium, fachschaft, fachschaftsmitglied, wahlperiode WHERE person.pid IN (SELECT DISTINCT person.pid FROM person, gremiumsmitglied, gremium, fachschaft, fachschaftsmitglied WHERE wahlperiode.wid = '" . $wahlperiode . "' AND gremiumsmitglied.wid = wahlperiode.wid AND gremiumsmitglied.pid = person.pid) AND fachschaftsmitglied.fid = fachschaft.fid AND person.pid = fachschaftsmitglied.pid AND person.pid = gremiumsmitglied.pid AND gremium.gid = gremiumsmitglied.gid ORDER BY gremiumsmitglied.gmid;";
